...
class TrainingPipe:
    ...
    def forward(self, x):
        ...
        if not self.is_last:
            comm.send(self.saved_output.shape, dest=self.rank+1, tag=10)
    ...
    def backward(self):
        if self.is_last:
            if not self.is_first:
                grad_input = self.saved_input.grad 
                if grad_input is not None:
                    print(f"Rank {self.rank} sending gradient to {self.rank-1}")
            ...
            print(f"Rank {self.rank} recieved gradients from {self.rank+1}")
            ...
            self.grad_output = torch.from_numpy(grad_numpy)
            self.saved_output.backward(self.grad_output, retain_graph=False)
            ...
            print(f"Rank {self.rank} recieved gradient from rank {self.rank+1}")
            shape = comm.recv(source=self.rank+1, tag=20)
            grad_numpy = np.empty(shape, dtype=np.float32)
            comm.Recv(grad_numpy, source=self.rank+1,tag=21)
...
if rank == 0:
    ...
    for epoch in range(10):
        ...
        for batch_idx, (x,y) in enumerate(train_dataloader):
            _ = new_model.forward(x)
            comm.Send(y.numpy(), dest=size-1, tag=99)
